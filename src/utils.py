...
# Fungsi menghitung heuristic dari global optimum, berdasarkan heuristic yang didapat
...
# Fungsi untuk inisialisasi bin (First Fit), [['id':(string)],['ukuran':(int)]],int -> [['kontainer':(string)], ['remaining':(int)], ['total':(int)], ['barang':[['id':(string)],['ukuran':(int)]]]
...
# Fungsi untuk memperbaiki urutan container, [dict] -> [dict]
def reindex(bin_rem):
  for index, b in enumerate(bin_rem):
    b['kontainer'] = f'Kontainer {index+1}'

# Procedure untuk mencetak isi bin
...
# Fungsi untuk menghitung Heuristic/Fitness function  [['kontainer':(string)], ['remaining':(int)], ['total':(int)], ['barang':[['id':(string)],['ukuran':(int)]]] -> int
...
# Fungsi untuk menshuffle weight pada worst fit [dict], int -> [dict]
...
# Fungsi untuk menshuffle weight pada worst fit [dict] -> [[dict]]
...
# Fungsi Steepest Ascent Hill Climbing [dict], int -> [dict]
...
# Fungsi Sideways Move Hill Climbing, [dict], int, -> [dict]
...
# Fungsi Random Restart Hill Climbing, [dict], int, -> [dict]
def RRHC_uc(bins, max_restart):
  reset_globals()
  restart = 0
  i = 0
  best_bin = bins
...
# Fungsi Stochastic Hill Climbing, [dict], int, -> [dict]
...
# Fungsi Simmulated Annealing, [dict], int, int -> [dict]
def simmulated_annealing_uc(bin, iterasi, T):
  ...
  for i in range(iterasi):
    ...
    # Tambahkan variabel dalam plot
    # constrain range agar tidak overflow
    try:
        plotE = math.exp(-deltaE / T)
    except ZeroDivisionError:
        plotE = 1
    except OverflowError:
        plotE = 0.0 
    # Probabilitas memiliki range 0 hingga 1 
    if (plotE > 1):
      plotE = 1
    if (plotE < 0):
      plotE = 0
    globals_data.display_heur.append((i, heuristic_uc(bin), plotE))
    # Turunkan suhu
    T = T * 0.95
    
  return bin

# Fungsi Genetic Algorithm, [dict], int, int, int, int, int, [dict]-> [dict]
...
# Fungsi Mutasi pada Genetic [['Fitness' : (int)]['bin' : (dict)]], int, -> [dict]
...
# Fungsi Mutasi pada Genetic ['Fitness' : (int)]['bin' : (dict)], int, [dict] -> ['Fitness' : (int)]['bin' : (dict)]
...
# Fungsi Crossover pada Genetic [['Fitness' : (int)]['bin' : (dict)]], int, int, [dict] -> [['Fitness' : (int)]['bin' : (dict)]]
